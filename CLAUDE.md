...
- **`currency.js`** — `window.BiocharCurrency`: FX rate fetcher (open.er-api.com, cached 24h in `biocharExchangeRates_v2`), USD-base `toBase`/`fromBase`/`format`, and selector builders (`wrapInput`, `injectOutputSelect`, `wrapClassInput`, `populateSelect`). After rates load, every currency selector contains the full world list; an offline fallback to 9 LatAm currencies is used if the FX fetch fails.
- **`project-currency.js`** — `window.BiocharProjectCurrency`: country list (ISO 3166-1 alpha-2), country → ISO 4217 currency map, legacy slug normalizer, `getProjectCurrency()` (reads `tool1.country` and resolves currency), `getCountryDisplayName(code, lang)` (via `Intl.DisplayNames`), and `applyHtmlDefaults(root)` which converts inputs marked `data-usd-default="N"` from USD to the project currency on first render of new projects.
...
- **Country selection drives the project default currency.** The Tool 1 country dropdown stores **ISO 3166-1 alpha-2** codes (e.g., `MX`, `BR`, `DE`); `BiocharProjectCurrency.getProjectCurrency()` resolves that to an ISO 4217 currency code. Every currency selector — field-level (`wrapInput`/`injectOutputSelect`), route-level (Tool 3 `wrapClassInput`), and page-level (Tool 6/7 `pageCurrency`) — defaults to the project currency when no explicit user choice has been saved. Tool 1's dropdown is populated at runtime from `BiocharProjectCurrency.COUNTRY_CODES` with localized names from `Intl.DisplayNames` — no per-country translation entries are maintained in `translations.js`.
- **Country-specific data fallback.** `tool5.html` `COUNTRY_DATA` and `tool6` `permit_<code>` translation keys exist only for the 13 LatAm countries (keyed by alpha-2) plus `custom`. Newly selectable countries fall through to `COUNTRY_DATA['custom']` / `permit_unknown`.
- **Legacy slugs.** Old saved projects may store `tool1.country` as snake-case slugs (`"mexico"`, `"brazil"`, ...). Always normalize with `BiocharProjectCurrency.normalizeCountryCode()` before looking up in `COUNTRY_DATA` or building a permit key.
- **Converting USD HTML defaults.** Inputs whose `value="..."` represents a USD default get a `data-usd-default="N"` attribute; `BiocharProjectCurrency.applyHtmlDefaults(scope)` converts them to the project currency on first paint of a *new* project. Only call this when `projectData.toolN.lastUpdated` is absent — never overwrite saved user data.
